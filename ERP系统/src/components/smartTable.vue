...
<div>
		...
		<input type="text" name="" v-model='tempFun'  :disabled='tmp.tdSelects.length<1' size="50" placeholder="请输入函数,例如A+B 或者(A0+B1-10)*10/2">
		<button @click='delFun'>清除函数</button>
...
export default {
   ...
   //  		//手工触发tdOver
    	...
    	createTable(){//创建表格
			let ts = this;
    		// 创建列
    		for(let i=0;i<ts.tmp.xNum;i++){
    			ts.addTD(1);
    		}
    		// 创建行
    		for(let i=0;i<ts.tmp.yNum;i++){
    			ts.addTD(2);
    		}
    	},
    	init(){//初始化
